...
export function queryPlanPrompt(brand,website,niche,count=5) {
...
Спочатку з'ясуй, що саме продає компанія. Кожен запит має стосуватись конкретної послуги/категорії товарів або окремої задачі покупця. Не роби п'ять перефразувань одного запиту «топ компаній у ніші». Для вузької спеціалізації розділи потреби, випадки використання або критерії вибору.
...
${JSON.stringify({brand,website,niche,count})}`;
}
