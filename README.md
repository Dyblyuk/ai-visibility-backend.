...
`verdict` — три стани замість простого "є/немає": **know** (пряма текстова
згадка бренду знайдена — найсильніший сигнал, довіряємо йому одразу, без
додаткового виклику), **confused** (прямої згадки немає, але класифікатор
бачить непряме/невиразне посилання — щось схоже, натяк, невпевненість),
**unknown** (жодної згадки чи натяку). `hit` лишається для зворотної
сумісності (`true` лише при `verdict: "know"`).

**Вартість:** класифікаційний виклик Claude робиться **лише тоді, коли
прямої текстової згадки бренду не знайдено** — тобто в гіршому випадку
+4 виклики на прямий запит (коли жодна система бренд не згадала явно), а
зазвичай менше. Якщо `ANTHROPIC_API_KEY` не налаштовано — класифікатор
недоступний, і бекенд чесно повертає `unknown` замість прямої згадки
(без `confused`, без вигаданих припущень).
