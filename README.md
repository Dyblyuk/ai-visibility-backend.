...
**Автовизначення ніші:** якщо користувач не вказав нішу/місто, `POST /api/discovery-queries`
сам через веб-пошук Claude визначає, чим займається бренд і де він розташований
(`inferNiche`), і будує нішеві запити вже на цій основі. Відповідь містить
`niche` (визначену чи введену нішу) і `inferred: true/false`, щоб фронтенд
міг чесно показати "AI сам визначив нішу: ...", а не видавати це за те, що
ввів користувач.
...
`verdict` — три стани замість простого "є/немає": **know** (класифікатор
підтверджує впевнене й точне знання бренду — і назва справді присутня
в тексті), **confused** (щось невиразне — натяк, невпевненість, плутанина
зі схожою назвою), **unknown** (жодної згадки, або асистент прямо каже,
що не має інформації — навіть якщо назву бренду він просто процитував
з питання). `hit` лишається для зворотної сумісності (`true` лише при
`verdict: "know"`).

**Вартість:** класифікаційний виклик Claude робиться **на кожну
перевірену систему** (4 виклики на прямий запит) — це свідомий вибір
на користь точності: проста перевірка підрядка плутає "я знаю бренд X"
з "я НЕ маю інформації про X" (коли AI просто повторює назву з питання,
відмовляючись відповісти). Якщо `ANTHROPIC_API_KEY` не налаштовано —
класифікатор недоступний, і бекенд падає назад на просту перевірку
підрядка (без `confused`, і без захисту від хибних спрацювань на
відмовах-з-цитуванням).
