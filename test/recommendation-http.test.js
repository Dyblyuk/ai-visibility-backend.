...
globalThis.fetch=async(url,options)=>{
 ...
 if(String(url).includes('anthropic.com'))return Response.json({content:[{type:'text',text:body.max_tokens===7000?JSON.stringify({engines:{chatgpt:extracted,claude:extracted,gemini:extracted,perplexity:extracted}}):raw}]});
...
test('discovery -> stored report -> SendPulse summary/PDF preserves per-AI evidence and cached scores',async()=>{
 ...
 try {
  const query='Порадь агенції для SEO у Києві.';
  ...
  for(const call of calls.filter(call=>call.body.max_tokens!==7000))assert.ok(!JSON.stringify(call.body).includes('Acme'));
  const cached=await post('/api/zone-query',{brand:'Acme',website:'acme.ua',query});assert.equal(calls.length,5);assert.equal(cached.engines.chatgpt.cached,true);
  const saved=await post('/api/save-report',{brand:'Acme',website:'acme.ua',niche:'SEO, Київ',score:5,engines:[{label:'ChatGPT',verdict:'know'}],zoneOfInvisibility:[zone]});
  ...
  assert.match(report.reportSummary,/ChatGPT: 100\/100 \(1\/1\)/);assert.ok(report.reportSummary.length<1024);
